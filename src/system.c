...
Parameters* setParameters(int argc, char** argv, ParametersL* param)
{
    ...
    else
    {
        ...
        if (!(param->geoFilePath[strlen(param->geoFilePath) - 1] == '/' || param->nameGeoFile[0] == '/'))
        {
    ...
    if (!(param->qryFilePath[strlen(param->qryFilePath) - 1] == '/' || param->nameQryFile[0] == '/'))
    {
    ...
    if (!(param->textFilePath[strlen(param->geoFilePath) - 1] == '/' || textFileName[0] == '/'))
    {
